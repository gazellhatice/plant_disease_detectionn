...
genai.configure(api_key="AIzaSyAP8ya9w1yoi_L7xZ_-SO2iDkoSaBD6dAE")
...
def download_pdf(prediction, confidence, advice, language):
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    story.append(Spacer(1, 20))
    story.append(Paragraph(f"<i>Generated on: {timestamp}</i>",
                           ParagraphStyle(name="Italic", fontSize=10, textColor=colors.grey)))
